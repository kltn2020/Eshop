defmodule EshopCore.Utils.StructHelper do
  ...
  def to_map(map) when is_map(map) do
    ...
    |> Enum.into(%{}, fn
      ...
      {key, %{__struct__: struct} = value} when struct in [DateTime] ->
        {key, EshopCore.Utils.DateTime.local_default(value)}

      {key, %{__struct__: struct} = value} when struct in [NaiveDateTime] ->
        {key,
         value |> DateTime.from_naive!("Etc/UTC") |> EshopCore.Utils.DateTime.local_default()}
